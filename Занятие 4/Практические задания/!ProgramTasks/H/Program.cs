namespace H
{
    internal class Program
    {
        ...
        {
            if (HasWinSequence(field, Mark.Circle) == HasWinSequence(field, Mark.Cross)) 
        ...
        static bool HasWinSequence(Mark[,] field, Mark mark)
        {
            ...
            for (int x = 0; x < 3; x++)
            {
                if ((field[0, x] & field[1, x] & field[2, x]) == mark)
                    return true;
                if ((field[x, 0] & field[x, 1] & field[x, 2]) == mark) 
                    return true;
            }
            return false;
        }
        ...
        private static Mark[,] CreateFromString(string str)
        {
            ...
            for (int x = 0; x < field.Length; x++)
                for (var y = 0; y < field.Length; y++)
                    marks[x, y] = field[x][y] == 'X' ? Mark.Cross : (field[x][y] == 'O' ? Mark.Circle : Mark.Empty);
            return marks;
        }
    }
}